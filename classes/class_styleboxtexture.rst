...
\ **注意：** GaaeExplorer 期望法线贴图使用 X+、Y- 和 Z+ 坐标系。参阅\ `这个页面 <http://wiki.polycount.com/wiki/Normal_Map_Technical_Details#Common_Swizzle_Coordinates>`__\ 比较流行引擎所期望的法线贴图坐标系。
