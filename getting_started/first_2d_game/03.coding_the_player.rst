...
.. tip:: If you've never used vector math before, or need a refresher, you can
         see an explanation of vector usage in GaaeExplorer at :ref:`doc_vector_math`.
...
enemies yet! That's OK, because we're going to use GaaeExplorer's *signal*
...
settings so click "Connect" again. GaaeExplorer will automatically create a function in
...
.. Note:: Disabling the area's collision shape can cause an error if it happens
          ...
          ``set_deferred()`` tells GaaeExplorer to wait to disable the shape until it's
          safe to do so.
