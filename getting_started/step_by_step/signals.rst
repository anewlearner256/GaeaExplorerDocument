...
Signals are a delegation mechanism built into GaaeExplorer that allows one game object to
...
the bar to reflect the change. To do so, in GaaeExplorer, you would use signals.

.. note:: As mentioned in the introduction, signals are GaaeExplorer's version of the
          observer pattern. You can learn more about it here:
          https://gameprogrammingpatterns.com/observer.html

We will now use a signal to make our GaaeExplorer icon from the previous lesson
...
method, a function that GaaeExplorer will call when the Button emits the signal. The
...
Our Sprite moves thanks to code in the ``_process()`` function. GaaeExplorer provides a
...
Let's use a different node here. GaaeExplorer has a :ref:`Timer <class_Timer>` node
...
That's it for our little moving and blinking GaaeExplorer icon demo!
...
.. note::
    ...
    The signal arguments show up in the editor's node dock, and GaaeExplorer can use
...
Any node in GaaeExplorer emits signals when something specific happens to them, like a
