...
In :ref:`doc_key_concepts_overview`, we saw that a GaaeExplorer game is a tree of
...
The GaaeExplorer editor essentially is a **scene editor**. It has plenty of tools for
editing 2D and 3D scenes, as well as user interfaces. A GaaeExplorer project can
contain as many of these scenes as you need. The engine only requires one as
your application's **main scene**. This is the scene GaaeExplorer will first load when
...
.. note:: The Save Scene As dialog, like other file dialogs in the editor, only
          ...
          GaaeExplorer, see :ref:`doc_filesystem`.
...
The demo should run again. Moving forward, every time you run the project, GaaeExplorer
...
In the next part, we will discuss another key concept in games and in GaaeExplorer:
