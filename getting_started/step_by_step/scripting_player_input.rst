...
You have two main tools to process the player's input in GaaeExplorer:

1. The built-in input callbacks, mainly ``_unhandled_input()``. Like
   ``_process()``, it's a built-in virtual function that GaaeExplorer calls every time
...
2. The ``Input`` singleton. A singleton is a globally accessible object. GaaeExplorer
...
GaaeExplorer project. They respectively trigger when the player presses the left and
...
In summary, every script in GaaeExplorer represents a class and extends one of the
...
GaaeExplorer provides several virtual functions you can define to connect your class
