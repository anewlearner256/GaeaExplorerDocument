...
to a back-buffer and then read from it while drawing. GaaeExplorer provides a
few tools that make this process easy.

SCREEN_TEXTURE built-in texture
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

GaaeExplorer :ref:`doc_shading_language` has a special texture, ``SCREEN_TEXTURE`` (and ``DEPTH_TEXTURE`` for depth, in the case of 3D).
...
The reason why textureLod must be used is because, when GaaeExplorer copies back
