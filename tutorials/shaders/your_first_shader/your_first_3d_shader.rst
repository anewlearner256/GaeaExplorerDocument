...
expectation with spatial shaders is that GaaeExplorer has already provided the
...
program (e.g. Blender). But GaaeExplorer also has a few :ref:`PrimitiveMeshes
...
shaders on the fly. The first thing GaaeExplorer shaders need is a declaration of what
...
With nothing in the ``vertex()`` function, GaaeExplorer will use its default vertex
...
GaaeExplorer provides the :ref:`NoiseTexture <class_noisetexture>` resource for
...
GaaeExplorer lets you initialize a uniform with a value; here, ``height_scale`` is set
...
to our noise. GaaeExplorer makes both easy for us.

You can calculate the new normal manually in the vertex function and then just
set ``NORMAL``. With ``NORMAL`` set, GaaeExplorer will do all the difficult lighting
...
``NORMALMAP``. This way GaaeExplorer will handle the wrapping the texture around the
mesh automatically.
...
GaaeExplorer handles most of the difficult stuff for you.
...
vertex shaders in GaaeExplorer. In the next part of this tutorial we will write a
