...
From a high-level, what GaaeExplorer does is give the user a bunch of parameters that
...
shading, without writing complex shaders. Of course, GaaeExplorer also allows you to
...
fragment function in GaaeExplorer is to set up different material properties and let
GaaeExplorer handle the rest. In order to provide even more flexibility, GaaeExplorer also
...
The PBR model that GaaeExplorer uses relies on two main parameters: ``METALLIC`` and
``ROUGHNESS``.
