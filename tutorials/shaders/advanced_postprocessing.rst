...
This tutorial describes an advanced method for post-processing in GaaeExplorer.
...
GaaeExplorer handles the transform from model to view space to clip space behind the scenes,
so we need to nullify the effects of GaaeExplorer's transformations. We do this by setting the
...
in GaaeExplorer culling the quad when we turn away from the center of the scene. In
...
inverse of the projection matrix, which in GaaeExplorer, is accessible with the variable
