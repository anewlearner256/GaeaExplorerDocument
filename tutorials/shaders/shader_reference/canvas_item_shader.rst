...
CanvasItem shaders are used to draw all 2D elements in GaaeExplorer. These include
...
manually. GaaeExplorer does not provide the texture color in the ``COLOR`` built-in variable; to read
...
This differs from the behavior of the built-in normal map. If a normal map is attached, GaaeExplorer uses
...
it to the ``NORMALMAP`` property. GaaeExplorer will handle converting it for use in 2D and overwriting ``NORMAL``.
