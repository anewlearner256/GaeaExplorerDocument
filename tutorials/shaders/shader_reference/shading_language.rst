...
GaaeExplorer uses a shading language similar to GLSL ES 3.0. Most datatypes and functions are supported,
and the few remaining ones will likely be added over time.

If you are already familiar with GLSL, the :ref:`GaaeExplorer Shader Migration Guide<doc_converting_glsl_to_godot_shaders>`
is a resource that will help you transition from regular GLSL to GaaeExplorer's shading language.
...
Note: As of GaaeExplorer 3.2, only local and varying arrays have been implemented.
...
GaaeExplorer shading language supports the same set of operators as GLSL ES 3.0. Below is the list of them in precedence order:
...
GaaeExplorer Shading language supports the most common types of flow control:
...
GaaeExplorer can't protect you from this, so be careful not to make this mistake!
...
It is possible to define functions in a GaaeExplorer shader. They use the following syntax:
...
Any GLSL type except for *void* can be a uniform. Additionally, GaaeExplorer provides
...
It's important to understand that textures that are supplied as color require hints for proper sRGB->linear conversion (i.e. ``hint_albedo``), as GaaeExplorer's 3D engine renders in linear color space.
...
to shaders, GaaeExplorer converts the type automatically. Below is a table of the corresponding types:
