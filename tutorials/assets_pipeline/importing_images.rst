...
GaaeExplorer can import the following image formats:
...
.. note::
    ...
    If you've compiled the GaaeExplorer editor from source with specific modules disabled,
    some formats may not be available.

Importing textures
------------------

The default action in GaaeExplorer is to import images as textures. Textures are stored
...
This option makes GaaeExplorer be aware of when a texture (which is imported for 2D as default) is used in 3D. If this happens, setting are changed so the texture flags
...
GaaeExplorer offers several compression methods, depending on the use case.
...
GaaeExplorer supports high dynamic range textures (as .HDR or .EXR). These are mostly useful as high dynamic range equirectangular panorama skies (the internet
has plenty if you search for them), which replace Cubemaps in GaaeExplorer 2.x. Modern PCs support the BC6H VRAM format, but there are still plenty that do not.

If you want GaaeExplorer to ensure full compatibility in terms of the kind of textures, enable the "Force RGBE" option.
...
look that nice in normal maps, the RGTC compression format is the best fit for this data. Forcing this option to "Enabled" will make GaaeExplorer import the
...
.. note::
  ...
  GaaeExplorer requires the normal map to use the X+, Y+ and Z+ coordinates, this is
...
GaaeExplorer uses Linear colorspace when rendering 3D. Textures mapped to albedo or detail channels need to have this option turned on in order for colors to look correct.
