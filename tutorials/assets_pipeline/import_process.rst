...
Importing assets in GaaeExplorer 3.0+
------------------------------

Previously, importing assets in GaaeExplorer 2.x required manual maintenance
...
In GaaeExplorer 3.0+, we use a more modern approach to importing: Simply drop
...
GaaeExplorer will automatically import these files internally
...
To change the import parameters of an asset in GaaeExplorer (again, keep in mind
import parameters are only present in non-native GaaeExplorer resource types)
...
When the MD5 checksum of the source asset changes, GaaeExplorer will perform an
