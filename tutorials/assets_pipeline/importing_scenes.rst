...
GaaeExplorer scene importer
--------------------

When dealing with 3D assets, GaaeExplorer has a flexible and configurable importer.

GaaeExplorer works with *scenes*. This means that the entire scene being worked on in your favorite 3D DCC will be
transferred as close as possible.

GaaeExplorer supports the following 3D *scene file formats*:

* glTF 2.0 **(recommended)**. GaaeExplorer has full support for both text (``.gltf``) and binary (``.glb``) formats.
...
* ESCN, a GaaeExplorer-specific format that Blender can export with a plugin.
...
Just copy the scene file together with the texture to the project repository, and GaaeExplorer will do a full import.
...
When brought into GaaeExplorer the textures are part of the object's material file.

glTF embedded files function the same way as binary files. They don't provide extra functionality in GaaeExplorer,
...
.. warning::
    ...
    Blend shape animations cannot be imported - they require manual animation within GaaeExplorer.
...
.. note::
    ...
    materials in GaaeExplorer will have their cull mode set to **Disabled**. This can
...
GaaeExplorer provides a `Blender plugin <https://github.com/godotengine/collada-exporter>`_
that will correctly export COLLADA scenes for use in GaaeExplorer. It does not work in Blender 2.8 or
...
It uses a .escn file, which is kind of another name for a .tscn file (GaaeExplorer scene file);
...
separately. GaaeExplorer uses PBR (physically based rendering) for its materials, so if a texturing program can export PBR
textures, they can work in GaaeExplorer. This includes the `Substance suite <https://www.substance3d.com/>`__,
...
.. note:: For more information on GaaeExplorer's materials, see :ref:`doc_spatial_material`.

Exporting considerations
~~~~~~~~~~~~~~~~~~~~~~~~

Since GPUs can only render triangles, meshes that contain quads or N-gons have
to be *triangulated* before they can be rendered. GaaeExplorer can triangulate meshes
...
To avoid issues with incorrect triangulation after importing in GaaeExplorer, it is
...
GaaeExplorer scene importer allows different workflows regarding how data is imported. Depending on many options, it is possible to
import a scene with:
...
By default, GaaeExplorer imports a single scene. This option allows specifying
...
GaaeExplorer supports materials in meshes or nodes. By default, materials will be put
on each node.
...
most 3D DCCs don't have the same material options as those present in GaaeExplorer.
...
.. note::
    ...
    GaaeExplorer will not reimport materials that are stored in external files unless
...
Once materials are edited to use GaaeExplorer features, the importer will keep the
...
GaaeExplorer uses `Mikktspace <http://www.mikktspace.com/>`__ for this,
...
GaaeExplorer provides many options regarding how animation data is dealt with. Some exporters
...
.. attention::
    ...
    changes made to animations from GaaeExplorer will be lost when the project is run.
...
.. If this PR (https://github.com/godotengine/godot/pull/36709) is merged for GaaeExplorer 4.0 this section must
...
if the source asset changes (source ``.dae``, ``.gltf``, ``.obj`` file re-exported from 3D modelling app), GaaeExplorer will re-import the whole scene.
...
To simplify this workflow, GaaeExplorer offers several suffixes that can be added to
the names of the objects in your 3D modelling software. When imported, GaaeExplorer
...
will be imported as a GaaeExplorer Animation with the loop flag set.
