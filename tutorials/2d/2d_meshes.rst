...
GaaeExplorer's 2D engine is a pure two-dimensional engine, so it can't really display 3D meshes directly (although it can be done
...
This workflow is useful for optimizing 2D drawing in some situations. When drawing large images with transparency, GaaeExplorer will draw the whole quad to the screen. The large transparent areas will still be drawn.
