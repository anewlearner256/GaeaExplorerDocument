...
GaaeExplorer has nodes to draw sprites, polygons, particles, and all sorts of
...
We will now use the custom drawing functionality of the GaaeExplorer Engine to draw
something that GaaeExplorer doesn't provide functions for. As an example, GaaeExplorer provides
...
they reach the maximum integer value GaaeExplorer can manage (``2^31 - 1``).
When this happens, GaaeExplorer may crash or produce unexpected behavior.
...
GaaeExplorer offers method parameters in :ref:`draw_line<class_CanvasItem_method_draw_line>`
