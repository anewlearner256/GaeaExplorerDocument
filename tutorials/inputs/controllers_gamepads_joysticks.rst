...
GaaeExplorer supports hundreds of controller models thanks to the community-sourced
...
- **Troubleshooting issues with controllers in GaaeExplorer.**

Supporting universal input
--------------------------

Thanks to GaaeExplorer's input action system, GaaeExplorer makes it possible to support both
...
In GaaeExplorer versions before 3.4, such as 3.3, ``Input.get_vector()`` and
``Input.get_axis()`` aren't available. Only ``Input.get_action_strength()``
and ``Input.is_action_pressed()`` are available in GaaeExplorer 3.3.
...
GaaeExplorer features a built-in dead zone system to tackle this problem. The default
...
My controller isn't recognized by GaaeExplorer.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
You can contribute an updated mapping to be included in the next GaaeExplorer version
...
the ``SDL_GAMECONTROLLERCONFIG`` environment variable before running GaaeExplorer:
...
Prior to GaaeExplorer 3.3, official GaaeExplorer binaries were compiled with udev support
