...
GaaeExplorer's scripting API is duck-typed. This means that if a script executes an
operation, GaaeExplorer doesn't validate that it supports the operation by **type**.
...
:ref:`CanvasItem.visible <class_CanvasItem_property_visible>`, then GaaeExplorer would do the
...
GaaeExplorer provides a variety of options for performing runtime checks on these
accesses:
...
These strategies contribute to GaaeExplorer's flexible design. Between them, users
