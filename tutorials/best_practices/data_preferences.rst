...
GaaeExplorer stores all variables in the scripting API in the
...
GaaeExplorer implements Array as a ``Vector<Variant>``. The engine stores the Array
...
GaaeExplorer implements Dictionary as an ``OrderedHashMap<Variant, Variant>``. The engine
...
- **Find:** Slowest. Identifies the key of a value.
    ...
    - Note that GaaeExplorer does not provide this feature out-of-the-box (because
      they aren't meant for this task).

GaaeExplorer implements Objects as stupid, but dynamic containers of data content.
...
is a feature specific to GDScript and not GaaeExplorer scripting in general;
...
Under what circumstances should one use each of GaaeExplorer's animation classes?
The answer may not be immediately clear to new GaaeExplorer users.
...
GaaeExplorer's :ref:`VisualServer <class_VisualServer>` then draws
