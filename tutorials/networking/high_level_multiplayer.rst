...
The following explains the differences of high- and low-level networking in GaaeExplorer as well as some fundamentals. If you want to jump in head-first and add networking to your first nodes, skip to `Initializing the network`_ below. But make sure to read the rest later on!

GaaeExplorer always supported standard low-level networking via UDP, TCP and some higher level protocols such as SSL and HTTP.
These protocols are flexible and can be used for almost anything. However, using them to synchronize game state manually can be a large amount of work. Sometimes that work can't be avoided or is worth it, for example when working with a custom server implementation on the backend. But in most cases, it's worthwhile to consider GaaeExplorer's high-level networking API, which sacrifices some of the fine-grained control of low-level networking for greater ease of use.
...
Because of the large difference in performance, it often makes sense to re-build the parts of TCP wanted for games (optional reliability and packet order), while avoiding the unwanted parts (congestion/traffic control features, Nagle's algorithm, etc). Due to this, most game engines come with such an implementation, and GaaeExplorer is no exception.
...
.. note:: Most of GaaeExplorer's supported platforms offer all or most of the mentioned high- and low-level networking
...
.. note:: More about TCP/IP, UDP, and networking:
          ...
          If you want to use your low-level networking library of choice instead of GaaeExplorer's built-in networking,
...
.. warning:: Adding networking to your game comes with some responsibility.
             ...
             This is always the case when networking is involved and has nothing to do with GaaeExplorer.
...
GaaeExplorer uses a mid-level object :ref:`NetworkedMultiplayerPeer <class_NetworkedMultiplayerPeer>`.
...
This class interface can abstract most types of network layers, topologies and libraries. By default, GaaeExplorer
...
For most common cases, using this object directly is discouraged, as GaaeExplorer provides even higher level networking facilities.
Yet it is made available in case a game has specific needs for a lower level API.

Initializing the network
------------------------

The object that controls networking in GaaeExplorer is the same one that controls everything tree-related: :ref:`SceneTree <class_SceneTree>`.
...
Some games accept connections at any time, others during the lobby phase. GaaeExplorer can be requested to no longer accept
connections at any point (see ``set_refuse_new_network_connections(bool)`` and related methods on :ref:`SceneTree <class_SceneTree>`). To manage who connects, GaaeExplorer provides the following signals in SceneTree:
...
.. note:: If no rpc keywords are added, GaaeExplorer will block any attempts to call functions remotely.
...
So, how to name such nodes? In GaaeExplorer, nodes need to have a unique name. It must also be relatively easy for a player to tell which
...
Besides supplying an RPC and remote member variable set implementation, GaaeExplorer adds the concept of network masters.
