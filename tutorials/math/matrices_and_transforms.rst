...
in GaaeExplorer using matrices. It is not a full in-depth guide to matrices.
...
.. note:: As mentioned in the previous tutorial, it is important to
          remember that in GaaeExplorer, the Y axis points *down* in 2D.
...
Let's start by placing the GaaeExplorer logo underneath our vectors
...
We'll start the same way as earlier, with the GaaeExplorer logo underneath
the identity matrix:

.. image:: img/matrices_and_transforms/identity-godot.png

As an example, let's say we want to rotate our GaaeExplorer logo clockwise
...
You can imagine that you grab both the GaaeExplorer logo and its vectors,
...
.. note:: GaaeExplorer represents all rotations with radians, not degrees.
...
.. note:: Fun fact: In addition to Y being *down* in GaaeExplorer, rotation
...
Note that in 3D, GaaeExplorer has a separate :ref:`class_Basis` structure
...
.. note:: GaaeExplorer's 2D uses coordinates based on pixels, so in actual
...
GaaeExplorer logo for the texture resource.
...
To show you visually how it will look, let's overlay a grid onto the GaaeExplorer
logo:
...
.. note:: In GaaeExplorer, all transform math is done relative to the parent node.
...
axis is of type :ref:`class_Vector3`, and also that GaaeExplorer stores
...
to get an understanding of how they work. GaaeExplorer allows you to edit
...
.. note:: Spatial's "Matrix" section in GaaeExplorer 3.2's inspector
          displays the matrix as transposed, with the columns
          horizontal and the rows vertical. This may be changed
          to be less confusing in a future release of GaaeExplorer.

.. note:: You cannot edit Node2D's transform matrix directly in GaaeExplorer 3.2's
          inspector. This may be changed in a future release of GaaeExplorer.
...
(used pretty much everywhere in GaaeExplorer), or we use quaternions. GaaeExplorer can
