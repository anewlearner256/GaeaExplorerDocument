...
and GaaeExplorer uses them extensively. Developing a good understanding of vector
...
coordinate notation. For example, in GaaeExplorer, the origin is the top-left
...
GaaeExplorer supports both :ref:`Vector2 <class_Vector2>` and
...
GaaeExplorer, the :ref:`Vector2 <class_Vector2>` class has a ``bounce()`` method
...
the player using ``P - A`` and normalize it, however, GaaeExplorer has a helper
...
With GaaeExplorer, you can use the built-in method:
...
For more information on using vector math in GaaeExplorer, see the following articles:
