...
In GaaeExplorer, materials are not transparent unless specifically configured to be.
...
For this reason, materials in GaaeExplorer are assumed opaque unless
...
GaaeExplorer has a more or less uniform cost per pixel thanks to depth pre-pass. All
...
textures, and channels. GaaeExplorer supports an extensive list of them. They are
...
previous versions of GaaeExplorer, this channel was named *Diffuse*. The change
...
GaaeExplorer uses a metallic model over competing models due to its simplicity.
...
This does not modify geometry, only the incident angle for light. In GaaeExplorer,
...
Some fabrics have small micro-fur that causes light to scatter around it. GaaeExplorer
...
The effect is extremely subtle in GaaeExplorer 3 releases, and may require specific
...
.. note:: The effect will be more noticeable in GaaeExplorer 4.
...
When refraction is enabled, it supersedes alpha blending, and GaaeExplorer attempts to
...
GaaeExplorer allows using secondary albedo and normal maps to generate a detail
...
GaaeExplorer supports two UV channels per material. Secondary UV is often useful for
...
GaaeExplorer allows materials to fade by proximity to each other as well as depending
