...
GaaeExplorer 3 provides a redesigned Environment resource, as well as a new
...
the screen where objects were not drawn). In GaaeExplorer 3.0, the background not only
...
physical effect is actually pretty complex, but GaaeExplorer provides a good approximation.
There are two kinds of fog in GaaeExplorer:
...
- **Mode:** The tone mapping mode to use.
  ...
  - **ACES:** *Deprecated (will be removed in GaaeExplorer 4.0).* Behaves like ACES,
...
GaaeExplorer supports a simple high dynamic range implementation with the auto exposure
...
While GaaeExplorer supports three sources of reflection data (Sky, ReflectionProbe, and
...
with ambient light. GaaeExplorer can simulate this using GIProbe, ReflectionProbe,
...
of the image. This is simulated in GaaeExplorer with the **Glow** effect.
...
To change the glow effect size and shape, GaaeExplorer provides **Levels**. Smaller
...
At the end of processing, GaaeExplorer offers the possibility to do some standard
image adjustments.
