...
normal GaaeExplorer array that is constructed with empty brackets ``[]``. It stores a ``Pool**Array``
...
all the arrays of data that the surface needs. GaaeExplorer will expect it to be of
...
GDScript, there is nothing GaaeExplorer specific about the approach to generating it.
