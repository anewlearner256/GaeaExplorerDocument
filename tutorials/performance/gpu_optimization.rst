...
instance in calculations inside the GaaeExplorer engine to prepare objects for
...
GaaeExplorer sends instructions to the GPU via a graphics API (OpenGL, OpenGL ES or
...
other). This can either be done by artists, or programmatically within GaaeExplorer.
...
The GaaeExplorer renderer is a little different to what is out there. It's designed to
...
possible. GaaeExplorer's priorities are:
...
By default, GaaeExplorer compresses textures of 3D models when imported using video RAM
