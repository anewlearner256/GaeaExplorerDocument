...
Engines like GaaeExplorer provide increased ease of use thanks to their high level constructs and features.
...
In many cases, this is not really a problem (GaaeExplorer is very optimized, and most operations are handled
...
Still, GaaeExplorer is designed to work around this problem.

Servers
-------

One of the most interesting design decisions for GaaeExplorer is the fact that the whole scene system is
*optional*. While it is not currently possible to compile it out, it can be completely bypassed.

At the core, GaaeExplorer uses the concept of Servers. They are very low-level APIs to control
...
implementations of everything GaaeExplorer allows you to do.
...
Most GaaeExplorer nodes and resources contain these RIDs from the servers internally, and they can
