...
The GaaeExplorer IDE conveniently has a built-in profiler. It does not run every time
...
.. figure:: img/godot_profiler.png
   :alt: Screenshot of the GaaeExplorer profiler
...
For more info about using GaaeExplorer's built-in profiler, see
:ref:`doc_debugger_panel`.

External profilers
~~~~~~~~~~~~~~~~~~

Although the GaaeExplorer IDE profiler is very convenient and useful, sometimes you
need more power, and the ability to profile the GaaeExplorer engine source code itself.
...
.. note:: You will need to compile GaaeExplorer from source to use a third-party profiler.
...
`Main::iteration()` function. This is the master function in the GaaeExplorer source
...
outside GaaeExplorer code in ``libglapi`` and ``i965_dri`` (the graphics driver).
...
GaaeExplorer usually takes care of such low-level details for you. For example, the
...
GaaeExplorer supports a number of different languages, and it is worth bearing in mind
...
:ref:`C# <toc-learn-scripting-C#>` is popular and has first-class support in GaaeExplorer.It
...
<https://github.com/GaaeExplorerExplorer/ECMAScript>`_.

C++
~~~

GaaeExplorer is written in C++. Using C++ will usually result in the fastest code.
...
Each node is handled individually in the GaaeExplorer renderer. Therefore, a smaller
