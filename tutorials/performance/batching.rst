...
As GaaeExplorer is designed to be a general-purpose engine, the primitives coming into
the GaaeExplorer renderer can be in any order, sometimes similar, and sometimes
dissimilar. To match GaaeExplorer's general-purpose nature with the batching
preferences of GPUs, GaaeExplorer features an intermediate layer which can
...
requiring few (if any) changes to your GaaeExplorer project.
...
In 3D, this is often exactly how engines work. However, in GaaeExplorer's 2D renderer,
...
In GaaeExplorer, this back-to-front order is determined by:
...
- :ref:`use_batching_in_editor
  ...
  Turns batching on or off in the GaaeExplorer editor.
...
- **batch R:** A batch containing rectangles. The second number is the number of
  rects. The second number in square brackets is the GaaeExplorer texture ID, and the
