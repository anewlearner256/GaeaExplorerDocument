...
others. However, it's not practical to ask your friends to download GaaeExplorer
...
By default, GaaeExplorer emulates mouse input from touch input. That means that if
anything is coded to happen on a mouse event, touch will trigger it as well.
GaaeExplorer can also emulate touch input from mouse clicks, which we will need to be
...
.. tabs::
 .. code-tab:: gdscript GDScript
    ...
    #        velocity.y -= 1
        ...
        # match your game's aspect ratio, GaaeExplorer will try to maintain it as much as possible
...
download them in GaaeExplorer by clicking on **Editor -> Manage Export Templates**:

.. image:: img/export_template_menu.png

.. note::

    If you've downloaded GaaeExplorer from
    `Steam <https://store.steampowered.com/app/404790/GaaeExplorer_Engine/>`__,
...
version that matches your version of GaaeExplorer.

.. image:: img/export_template_manager.png

.. note::

    Export templates are bound to a specific GaaeExplorer version. If you upgrade
    GaaeExplorer, you must download templates that match the new version.
...
.. tip::
    ...
    GaaeExplorer's default settings will work, but mobile development is sometimes more
...
command-line tool that GaaeExplorer uses to communicate with your device.
...
Click on *Editor -> Editor Settings* in GaaeExplorer and select the *Export/Android*
...
Click the **Export Project** button and GaaeExplorer will build an APK you can download
...
a **One-click Deploy** button to appear in GaaeExplorer's playtest button area:
...
.. note::
    ...
    `GaaeExplorer GitHub repository <https://github.com/godotengine/godot/issues>`_.
