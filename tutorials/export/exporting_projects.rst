...
Originally, GaaeExplorer did not have any means to export projects. The
...
Distributing a game project on PC with GaaeExplorer is rather easy. Drop
the GaaeExplorer binary in the same directory as the ``project.godot`` file,
...
Finally, GaaeExplorer has a simple but efficient system for
...
tools (SDKs) to be installed to be able to export. Additionally, GaaeExplorer
...
When exporting, GaaeExplorer makes a list of all the files to export and then
...
In production, it is useful to automate builds, and GaaeExplorer supports this
...
The output file extension should match the one used by the GaaeExplorer export process:
...
a single exported main pack file to be used with multiple GaaeExplorer executables.
...
.. seealso::
    ...
    See :ref:`doc_command_line_tutorial` for more information about using GaaeExplorer
...
- Not readable and writable using tools normally present on the user's
  ...
  `third-party tools <https://github.com/hhyyrylainen/GaaeExplorerPckTool>`__
