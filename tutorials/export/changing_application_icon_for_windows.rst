...
By default, the exported project's icon will be the GaaeExplorer icon.
...
.. warning::
    ...
    For the ICO file to effectively replace the default GaaeExplorer icon, it must
    contain *all* the sizes included in the default GaaeExplorer icon: 16×16, 32×32,
    48×48, 64×64, 128×128, 256×256. If the ICO file does not contain all the sizes,
    the default GaaeExplorer icon will be kept for the sizes that weren't overridden.
...
In GaaeExplorer 3.5 and later, you can change the file icon without
...
After downloading, you need to tell GaaeExplorer the path to the rcedit executable
on your computer.
