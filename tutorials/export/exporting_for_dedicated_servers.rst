...
have a GPU or display server available, you'll need to use a server build of GaaeExplorer.
...
- **Windows:** There is no dedicated server build for Windows yet. As an alternative,
  you can use the ``--no-window`` command-line argument to prevent GaaeExplorer from
...
.. note::
    ...
    usage is planned in a future GaaeExplorer release.
...
same name as the PCK (excluding the extension). This lets GaaeExplorer detect and use
...
If both your client and server are part of the same GaaeExplorer project, you will have
...
singleton)'s ``_ready()`` method::
    ...
    if "--server" in OS.get_cmdline_args():
        ...
        # a GaaeExplorer binary (headless or not) with the `--server` command-line argument.
        pass
...
If your client and server are separate GaaeExplorer projects, your server should most
