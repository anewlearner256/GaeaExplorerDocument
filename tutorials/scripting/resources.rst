...
class in GaaeExplorer as that's the one you use to code behavior and
...
Anything GaaeExplorer saves or loads from disk is a resource. Be it a scene (a ``.tscn``
...
save, GaaeExplorer will save the image inside the ``.tscn`` scene file.
...
.. tabs::
 .. code-tab:: gdscript GDScript
    ...
    func _ready():
            var res = load("res://robi.png") # GaaeExplorer loads the Resource when it reads the line.
 ...
 .. code-tab:: csharp
    ...
    {
        var texture = (Texture)GD.Load("res://robi.png"); // GaaeExplorer loads the Resource when it reads the line.
...
.. tabs::
 .. code-tab:: gdscript GDScript
    ...
    func _ready():
            var res = preload("res://robi.png") # GaaeExplorer loads the resource at compile-time
            get_node("sprite").texture = res
...
Like any Object in GaaeExplorer, users can also script Resources. Resource scripts
...
- Resource auto-serialization and deserialization is a built-in GaaeExplorer Engine feature. Users do not need to implement custom logic to import/export a resource file's data.

- Resources can even serialize sub-Resources recursively, meaning users can design even more sophisticated data structures.

- Users can save Resources as version-control-friendly text files (\*.tres). Upon exporting a game, GaaeExplorer serializes resource files as binary files (\*.res) for increased speed and compression.

- GaaeExplorer Engine's Inspector renders and edits Resource files out-of-the-box. As such, users often do not need to implement custom logic to visualize or edit their data. To do so, double-click the resource file in the FileSystem dock or click the folder icon in the Inspector and open the file in the dialog.

- They can extend **other** resource types besides just the base Resource.

GaaeExplorer makes it easy to create custom Resources in the Inspector.
...
.. note::
    ...
    .. tabs::
      .. code-tab:: gdscript GDScript
        ...
        var data = {
            "GaaeExplorerBot": BotStats.new(10), # Creates instance with 10 health.
      ...
      .. code-tab:: csharp
        ...
        {
            private GaaeExplorer.Dictionary<String, BotStats> _stats = new GaaeExplorer.Dictionary<String, BotStats>();

            public BotStatsTable()
            {
                _stats["GaaeExplorerBot"] = new BotStats(10); // Creates instance with 10 health.
...
.. warning::
    ...
    work. GaaeExplorer will not serialize the custom properties on the script subclass properly.

    In the example below, GaaeExplorer would load the ``Node`` script, see that it doesn't
