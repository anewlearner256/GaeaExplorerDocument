...
additions to GaaeExplorer, using interfaces such as
...
1) A GaaeExplorer executable for your target version.
...
3) A Git clone of the `godot-headers
   ...
   the C headers for GaaeExplorer's public API exposed to GDNative.
...
.. note::
    ...
    The ``godot-headers`` repository has different branches. As GaaeExplorer evolves,
    ...
    GaaeExplorer stable branch (e.g. ``3.4``) and ideally actual release (e.g.
    ``3.4.4-stable``) that you use.
    GDNative modules built against older versions of the GaaeExplorer headers *may*
...
the ``master`` branch of GaaeExplorer and thus contains the GDNative class and
structure definitions that will work with the latest development builds.

If you want to write a GDNative module for a stable version of GaaeExplorer, look at
...
GaaeExplorer 3.4.4. In your cloned repository, you can do:
...
If you are building GaaeExplorer from source with your own changes that impact
...
Open up GaaeExplorer and create a new project called "simple" alongside your
...
collection of functions for calling back into the main GaaeExplorer executable. In
...
A NativeScript behaves like any other script in GaaeExplorer. Because the NativeScript
...
Next up is the first of the entry points GaaeExplorer will call when our dynamic
...
that initializes our dynamic library. GaaeExplorer will give it a pointer to a
...
unloaded. GaaeExplorer will unload the library when no object uses it anymore. Here,
...
need today. This function will be called by GaaeExplorer as part of loading a GDNative
...
of object in GaaeExplorer that we 'inherit' from; this is not true inheritance but it's
close enough. Finally, our fourth and fifth parameters are descriptions for our
constructor and destructor.

We then tell GaaeExplorer about our methods (well our one method in this case), by
...
memory for our structure and fill it with some data. Note that we use GaaeExplorer's
...
Our destructor is called when GaaeExplorer is done with our object and we free our
...
need to convert our C string to a GaaeExplorer string object, and then copy that string
...
Strings are heap-allocated in GaaeExplorer, so they have a destructor which frees the
...
never deallocated. The returned variant gets automatically destroyed by GaaeExplorer.
...
The variant we return is destroyed automatically by GaaeExplorer.
...
extension which we place alongside our dynamic libraries. This file tells GaaeExplorer
what dynamic libraries are part of our module and need to be loaded per
platform.

We can use GaaeExplorer to generate this file, so open the "simple" project in the
editor.
...
The *Dependencies* column (also ``dependencies`` section) tells GaaeExplorer what other
...
With our ``.gdnlib`` file we've told GaaeExplorer how to load our library, now we need
...
After all that, our project should work. The first time you run it GaaeExplorer will
