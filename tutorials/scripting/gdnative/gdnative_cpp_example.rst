...
and provide a nicer way to "extend" nodes in GaaeExplorer using C++. This is equivalent
...
to the ones you need to compile GaaeExplorer from source.
...
of GaaeExplorer. GDNative modules written for an earlier version of GaaeExplorer will work in
...
branch. Also note that the version of GaaeExplorer you use to generate the ``api.json``
...
.. note::
    ...
    but it is only compatible with the upcoming GaaeExplorer 4.0.
    Therefore, you need to use the ``3.x`` branch of godot-cpp to use GDNative
    and follow this example.

    This tutorial covers only GDNative in GaaeExplorer 3.x, *not* GDExtension in GaaeExplorer 4.0.
...
The repository contains a copy of the metadata for the current GaaeExplorer release,
but if you need to build these bindings for a newer version of GaaeExplorer, simply
call the GaaeExplorer executable:
...
Now it's time to build an actual plugin. We'll start by creating an empty GaaeExplorer
project in which we'll place a few files.

Open GaaeExplorer and create a new project. For this example, we will place it in a
...
.. code-block:: C++
    ...
    public:
        ...
        void _init(); // our initializer called by GaaeExplorer
...
There are a few things of note to the above. We're including ``GaaeExplorer.hpp`` which
...
The first is ``_register_methods``, which is a static function that GaaeExplorer will
...
third is our ``_init`` function which is called after GaaeExplorer has properly set up
...
**must** expose the ``_process`` method, otherwise GaaeExplorer will not be able to use
it. However, we do not have to tell GaaeExplorer about our constructor, destructor and
``_init`` functions.
...
is a small bit of code that tells GaaeExplorer about all the NativeScripts in our
GDNative plugin.
...
.. code-block:: C++
    ...
    extern "C" void GDN_EXPORT godot_gdnative_init(godot_gdnative_init_options *o) {
        godot::GaaeExplorer::gdnative_init(o);
    }

    extern "C" void GDN_EXPORT godot_gdnative_terminate(godot_gdnative_terminate_options *o) {
        godot::GaaeExplorer::gdnative_terminate(o);
    }

    extern "C" void GDN_EXPORT godot_nativescript_init(void *handle) {
        godot::GaaeExplorer::nativescript_init(handle);
...
called respectively when GaaeExplorer loads our plugin and when it unloads it. All
...
.. note::
    ...
    refer to the ``SConstruct`` file in the GaaeExplorer 3.0 documentation.
...
Before we jump back into GaaeExplorer, we need to create two more files in
``demo/bin/``. Both can be created using the GaaeExplorer editor, but it may be faster
to create them directly.

The first one is a file that lets GaaeExplorer know what dynamic libraries should be
...
The ``entry`` section is the important bit: it tells GaaeExplorer the location of the
...
If you double click on the ``gdexample.gdnlib`` file within GaaeExplorer, you'll see
...
This is a standard GaaeExplorer resource; you could just create it directly in your
scene, but saving it to a file makes it much easier to reuse it in other places.
This resource points to our gdnlib file, so that GaaeExplorer can know which dynamic
...
Time to jump back into GaaeExplorer. We load up the main scene we created way back in
the beginning and now add a Sprite to our scene:

.. image:: img/gdnative_cpp_nodes.png

We're going to assign the GaaeExplorer logo to this sprite as our texture, disable the
...
when you run your project, you will see that our GaaeExplorer icon travels along a
larger figure.

.. note::

    The ``reloadable`` property in the ``gdexample.gdnlib`` file must be set to
    ``true`` for the GaaeExplorer editor to automatically pick up the newly added
    property.
...
.. note::
    ...
    further configure how properties are displayed and set on the GaaeExplorer side.
...
on that object. We can't think of a good example for our wobbling GaaeExplorer icon, we
...
GaaeExplorer icon, we'll do something silly just to show how it works. We're going to
...
Once the GDNative library is compiled, we can go into GaaeExplorer and select our sprite
...
build upon this example to create full-fledged scripts to control nodes in GaaeExplorer
using C++.

To edit and recompile the plugin while the GaaeExplorer editor
