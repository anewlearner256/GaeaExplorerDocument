...
to access a member from the base class ``GaaeExplorer.Object``, like ``Connect``.
For such use cases we provide a static property named ``Singleton`` that returns
the singleton instance. The type of this instance is ``GaaeExplorer.Object``.
...
GDScript               C#
=====================  ==============================================================
``Array``              ``GaaeExplorer.Collections.Array``
``PoolIntArray``       ``int[]``
``PoolByteArray``      ``byte[]``
...
``GaaeExplorer.Collections.Array<T>`` is a type-safe wrapper around ``GaaeExplorer.Collections.Array``.
Use the ``GaaeExplorer.Collections.Array<T>(GaaeExplorer.Collections.Array)`` constructor to create one.

Dictionary
----------

Use ``GaaeExplorer.Collections.Dictionary``.

``GaaeExplorer.Collections.Dictionary<T>`` is a type-safe wrapper around ``GaaeExplorer.Collections.Dictionary``.
Use the ``GaaeExplorer.Collections.Dictionary<T>(GaaeExplorer.Collections.Dictionary)`` constructor to create one.
...
The equivalent of yield on signal can be achieved with async/await and ``GaaeExplorer.Object.ToSignal``.
