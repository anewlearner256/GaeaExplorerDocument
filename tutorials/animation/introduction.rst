...
In GaaeExplorer, you can animate anything available in the Inspector, such as
...
.. figure:: img/animation_keyframes.png
   :alt: Keyframes in GaaeExplorer

   Keyframes in GaaeExplorer
...
between two points on the screen. For this tutorial, use the default GaaeExplorer
...
property, GaaeExplorer asks whether it should set it up for us. Click **Create**.
...
GaaeExplorer has an additional feature here. Like said before,
GaaeExplorer always calculates the frames between two keyframes. In a loop, the
...
The update mode of a track tells GaaeExplorer when to update the property
values. This can be:
...
The interpolation tells GaaeExplorer how to calculate the frame values between
...
GaaeExplorer supports two loop modes, which affect the animation if it's set to
loop:
...
-  Wrap loop interpolation: When this is selected, GaaeExplorer calculates the
...
GaaeExplorer doesn't restrict you to only edit transform properties. Every
...
this button and GaaeExplorer automatically adds a track and keyframe to the
current animation.
...
clicking and dragging the easing setting. This tells GaaeExplorer, how to change
...
GaaeExplorer's animation engine doesn't stop here. If you're already
comfortable with GaaeExplorer's scripting language
...
GaaeExplorer should call when it reaches this keyframe.
...
window. GaaeExplorer adds the track with the reference to the node.
...
Right click the timeline where GaaeExplorer should play the sample and
...
When GaaeExplorer reaches the keyframe, GaaeExplorer calls the
...
This does not apply on tracks created with GaaeExplorer versions prior to 3.4,
