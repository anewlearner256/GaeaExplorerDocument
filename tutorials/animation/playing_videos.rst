...
GaaeExplorer supports video playback with the :ref:`class_VideoPlayer` node.
...
H.264 and H.265 cannot be supported in core GaaeExplorer, as they are both encumbered
...
WebM is supported in core in GaaeExplorer 3.x, but support for it will be removed in 4.0
...
.. note::
    ...
    imported in GaaeExplorer. However, not all files with ``.ogg`` or ``.ogx``
...
By default in GaaeExplorer 4.0, the VideoPlayer will automatically be resized to match
...
There are several limitations with the current implementation of video playback in GaaeExplorer:
