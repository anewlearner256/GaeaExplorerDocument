...
With :ref:`AnimationPlayer <class_AnimationPlayer>`, GaaeExplorer has one of the most flexible animation systems that you can find in any game engine.
...
:ref:`AnimationTree <class_AnimationTree>` is a new node introduced in GaaeExplorer 3.1 to deal with advanced transitions.
...
At the end, the imported GaaeExplorer scene will contain the animations in a ``AnimationPlayer`` node.

As you rarely use imported scenes directly in GaaeExplorer (they are either instantiated or inherited from), you can place the ``AnimationTree`` node in your
...
When playing back the animation in GaaeExplorer, it is possible to select this bone as the *root motion track*. Doing so will cancel the bone
...
One of the nice features in GaaeExplorer's ``StateMachine`` implementation is the ability to travel. The graph can be instructed to go from the
