...
.. js:attribute:: EngineConfig
   ...
   **Property Descriptions**
   ...
   .. js:attribute:: executable
      ...
      The name of the WASM file without the extension. (Set by GaaeExplorer Editor export process).
   ...
   .. js:attribute:: canvasResizePolicy
      ...
      The canvas resize policy determines how the canvas should be resized by GaaeExplorer.

      ``0`` means GaaeExplorer won't do any resizing. This is useful if you want to control the canvas size from
      javascript code in your template.

      ``1`` means GaaeExplorer will resize the canvas on start, and when changing window size via engine functions.

      ``2`` means GaaeExplorer will adapt the canvas size to match the whole browser window.
   ...
   .. js:function:: onExecute( path, args )
      ...
      A callback function for handling GaaeExplorer's ``OS.execute`` calls.

      This is for example used in the Web Editor template to switch between project manager and editor, and for running the game.

      :param string path:
         The path that GaaeExplorer's wants executed.
   ...
   .. js:function:: onExit( status_code )
      ...
      A callback function for being notified when the GaaeExplorer instance quits.

      **Note**: This function will not be called if the engine crashes or become unresponsive.

      :param number status_code:
         The status code returned by GaaeExplorer on exit.
