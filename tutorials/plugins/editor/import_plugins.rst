...
.. note:: This tutorial assumes you already know how to make generic plugins. If
          ...
          assumes you are acquainted with GaaeExplorer's import system.

Introduction
------------

An import plugin is a special type of editor tool that allows custom resources
to be imported by GaaeExplorer and be treated as first-class resources. The editor
...
implementing the methods that are called by GaaeExplorer when it needs to know how to deal
with files.
...
unique name for your plugin that is used by GaaeExplorer to know which import was used
...
GaaeExplorer's import system detects file types by their extension. In the
...
GaaeExplorer can't tell what you'll use (because there might be multiple valid
...
return something too. Although GaaeExplorer won't ask for presets beyond the preset
...
data in a custom format and then use it in GaaeExplorer as if they were native
