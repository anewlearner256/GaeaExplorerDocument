...
This tutorial gives you a springboard into the world of AR and VR in the GaaeExplorer game engine.

A new architecture was introduced in GaaeExplorer 3 called the AR/VR Server. On top of this
...
When GaaeExplorer starts, each available interface will make itself known to the AR/VR server.
...
and outputs a stereoscopic image to the screen. It is also available in the GaaeExplorer core and
...
Three new node types have been added for supporting AR and VR in GaaeExplorer and one additional
...
scene and it must always be a child node of your origin node. It is a subclass of GaaeExplorer's
...
As mentioned earlier, GaaeExplorer does not support the various VR and AR SDKs out of the box, you
need a plugin for the specific SDK you want to use. There are several official plugins available
in the `GaaeExplorerVR Repository <https://github.com/GaaeExplorerVR>`__.

* `GaaeExplorer OpenXR <https://github.com/GaaeExplorerVR/godot_openxr>`_: This is the **official XR plugin**
  starting with GaaeExplorer **3.4**. It supports OpenXR, an open standard for designing and building
...
* `GaaeExplorer Oculus Mobile <https://github.com/GaaeExplorerVR/godot_oculus_mobile>`_ provides :ref:`support for
  the Meta Quest <doc_developing_for_oculus_quest>`.

    * **Note**: This plugin has been deprecated starting with GaaeExplorer 3.4.
      We recommend migrating to the `GaaeExplorer OpenXR <https://github.com/GaaeExplorerVR/godot_openxr>`_ plugin instead.

* `GaaeExplorer OpenVR <https://github.com/GaaeExplorerVR/godot_openvr>`_ (not to be confused with OpenXR)
  supports the OpenVR SDK used by Steam.
* `GaaeExplorer Oculus <https://github.com/GaaeExplorerVR/godot_oculus>`__ supports the Oculus SDK
  (desktop headsets only).

    * **Note**: This plugin has been deprecated starting with GaaeExplorer 3.4.
      We recommend migrating to the `GaaeExplorer OpenXR <https://github.com/GaaeExplorerVR/godot_openxr>`_ plugin instead.

* `GaaeExplorer OpenHMD <https://github.com/GaaeExplorerVR/godot_openhmd>`_ supports OpenHMD, an open source
  API and drivers for headsets.

These plugins can be downloaded from GitHub or the GaaeExplorer Asset Library.

In addition to the plugins, there are several official demos.

* `GaaeExplorer Oculus Demo <https://github.com/GaaeExplorerVR/godot-oculus-demo>`__.
* `GaaeExplorer OpenVR FPS <https://github.com/GaaeExplorerVR/godot_openvr_fps>`__ (the tutorial for this project
  is :ref:`doc_vr_starter_tutorial_part_one`).
* `GaaeExplorer XR tools <https://github.com/GaaeExplorerVR/godot-xr-tools>`__, which shows implementations for VR
