...
This tutorial will show you how to make a beginner VR game project in GaaeExplorer.
...
- How to tell GaaeExplorer to run in VR.
...
.. tip:: While this tutorial can be completed by beginners, it is highly
          ...
          if you are new to GaaeExplorer and/or game development.

          **Some experience with making 3D games is required** before going through this tutorial series.
          This tutorial assumes you have experience with the GaaeExplorer editor, GDScript, and basic 3D game development.
...
The GaaeExplorer project for this tutorial is found on the `OpenVR GitHub repository <https://github.com/GaaeExplorerVR/godot_openvr_fps>`__. The starter assets for this tutorial can be found in the releases
...
.. note:: **Credits for the assets provided**:
          ...
          - The OpenVR addon was created by `Bastiaan Olij <https://github.com/BastiaanOlij>`__ and is released under the MIT license. It can be found both on the `GaaeExplorer Asset Library <https://godotengine.org/asset-library/asset/150>`__ and on `GitHub <https://github.com/GaaeExplorerVR/godot-openvr-asset>`__. *3rd party code and libraries used in the OpenVR addon may be under a different license.*
...
.. tip:: You can find the finished project on the `OpenVR GitHub repository <https://github.com/GaaeExplorerVR/godot_openvr_fps>`__.


Getting everything ready
------------------------

If you have not already, go to the `OpenVR GitHub repository <https://github.com/GaaeExplorerVR/godot_openvr_fps/releases/>`__ and download the "Starter Assets" file from the releases. Once you have the
starter assets downloaded, open up the project in GaaeExplorer.
...
.. note:: The OpenVR VR interface is not included with GaaeExplorer by default. You will need to download the OpenVR asset from the
          `Asset Library <https://godotengine.org/asset-library/asset/150>`__ or `GitHub <https://github.com/GaaeExplorerVR/godot-openvr-asset>`__.
...
whether the OpenVR interface was able to initialize or not. If both of these conditionals return true, then we can turn the main GaaeExplorer :ref:`Viewport <class_Viewport>` into
an ARVR viewport.

If the VR interface initialized successfully, we then get the root :ref:`Viewport <class_Viewport>` and set the `arvr` property to ``true``. This will tell GaaeExplorer to use the initialized
ARVR interface to drive the :ref:`Viewport <class_Viewport>` display.

Finally, we disable VSync so the Frames Per Second (FPS) is not capped by the computer monitor. After this we tell GaaeExplorer to render at ``90`` frames per second, which is the
...
That is all we need to do for GaaeExplorer to launch OpenVR within the project! Go ahead and give it a try if you want. Assuming everything works, you will be able to look around
...
First we tell GaaeExplorer to silence the warnings about not using the values returned by the ``connect`` function. We will not need the returned
...
the :ref:`ARVRCamera <class_ARVRCamera>`, in world space. These vectors point in the same direction of the blue and red arrows when you select an object in the GaaeExplorer editor with
...
First we start the script with ``class_name VR_Interactable_Rigidbody``. What this does is that it tells GaaeExplorer that this GDScript is a new class that called ``VR_Interactable_Rigidbody``.
...
the class just like all of the built-in GaaeExplorer classes.
...
Finally, we have four functions. The ``_ready`` function is defined by GaaeExplorer and all we do is simply have ``pass`` as there is nothing we need to do when the object is added to the scene
...
`GaaeExplorer demo repository <https://github.com/godotengine/godot-demo-projects>`__.
...
.. warning:: You can download the finished project for this tutorial series on the GaaeExplorer OpenVR GitHub repository, under the releases tab!
