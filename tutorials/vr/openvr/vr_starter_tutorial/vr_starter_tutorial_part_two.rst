...
``Local space mode`` is enabled in the GaaeExplorer editor. We store this direction in a variable called ``direction_vector``.
...
help serve as an introduction to making fully-featured VR games in GaaeExplorer! The code and concepts detailed in this tutorial can be expanded on to make puzzle games, action games,
story-based games, and more!

.. warning:: You can download the finished project for this tutorial series on the `OpenVR GitHub repository <https://github.com/GaaeExplorerVR/godot_openvr_fps>`__, under the releases tab!
