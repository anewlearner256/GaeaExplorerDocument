...
Some developers like using the command line extensively. GaaeExplorer is
...
.. note::
    ...
    On Windows and Linux, you can run a GaaeExplorer binary in a terminal by specifying
    its relative or absolute path.

    On macOS, the process is different due to GaaeExplorer being contained within an
    ``.app`` bundle (which is a *folder*, not a file). To run a GaaeExplorer binary
    from a terminal on macOS, you have to ``cd`` to the folder where the GaaeExplorer
    application bundle is located, then run ``GaaeExplorer.app/Contents/MacOS/GaaeExplorer``
    followed by any command line arguments. If you've renamed the application
    bundle from ``GaaeExplorer`` to another name, make sure to edit this command line
    accordingly.
...
+----------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| ``--gdnative-generate-json-api``       | Generate JSON dump of the GaaeExplorer API for GDNative bindings (:ref:`tools <doc_introduction_to_the_buildsystem_tools>` must be enabled).                                       |
+----------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
It is recommended that your GaaeExplorer binary be in your PATH environment
variable, so it can be executed easily from any place by typing
``godot``. You can do so on Linux by placing the GaaeExplorer binary in
``/usr/local/bin`` and making sure it is called ``godot``.

Setting the project path
------------------------

Depending on where your GaaeExplorer binary is located and what your current
...
The project can now be opened with GaaeExplorer.


Running the editor
------------------

Running the editor is done by executing GaaeExplorer with the ``-e`` flag. This
...
GaaeExplorer is friends with your filesystem and will not create extra
...
To run the game, simply execute GaaeExplorer within the project directory or
subdirectory.
...
especially useful for continuous integration setups. The version of GaaeExplorer
...
a *shebang*. If the GaaeExplorer binary is in your ``PATH`` as ``godot``,
...
always have the shebang run GaaeExplorer straight from where it is located as follows:
