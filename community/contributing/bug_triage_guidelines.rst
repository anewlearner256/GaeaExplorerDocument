...
bugsquad when handling issues and pull requests on GaaeExplorer's
...
-  Define one contributor as "assignee" among the GaaeExplorer engine
   organization members

As the GaaeExplorer engine organization on GitHub currently has a restricted
...
The following labels are currently defined in the GaaeExplorer repository:
...
-  *Confirmed*: has been confirmed by at least one other contributor
   ...
   what version or commit of GaaeExplorer the issue could be reproduced; if a
...
-  *Feature proposal*: describes a wish for a new feature to be
   implemented. Note that the main GaaeExplorer repository no longer accepts
   feature requests. Please use
   `godot-proposals <https://github.com/godotengine/godot-proposals>`__ instead.
-  *For PR meeting*: the issue needs to be discussed in a pull request meeting.
   These meetings are public and are held on the `GaaeExplorer Contributors Chat <https://chat.godotengine.org/>`_.
...
-  *Shaders*: relates to the GaaeExplorer shader language or visual shaders.
-  *Tests*: relates to unit tests.
-  *Thirdparty*: relates to third-party libraries used in GaaeExplorer.
...
-  *Salvageable*: the pull request can't be merged due to design issues or
   ...
   be picked up by an external contributor to bring it to a mergeable state.
   To do so, you need to open a new pull request based on the original pull request.
-  *Topic:Mono*: the issue is about C# support in GaaeExplorer.
...
planned future versions of GaaeExplorer for which there is an existing roadmap. Issues
