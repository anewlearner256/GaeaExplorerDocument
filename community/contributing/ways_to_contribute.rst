...
GaaeExplorer Engine is a non-profit, community-driven free and open source project.
...
This means that to thrive, GaaeExplorer needs as many users as possible to get
...
-  **Be part of the community.** The best way to contribute to GaaeExplorer and help
...
-  **Make games.** It's no secret that, to convince new users and especially the
   industry at large that GaaeExplorer is a relevant market player, we need great games
   made with GaaeExplorer. We know that the engine has a lot of potential, both for 2D
   and 3D games, but given its young age we still lack big releases that will
   draw attention to GaaeExplorer. So keep working on your awesome projects, each new
...
-  **Donate.** GaaeExplorer is a non-profit project, but it can still benefit from
...
GaaeExplorer's `MIT <https://tldrlegal.com/license/mit-license>`_ license grants you,
...
`GaaeExplorer's source code <https://github.com/godotengine/godot>`_, and send those
...
To ensure good collaboration and overall quality, the GaaeExplorer developers
...
as it is expected that in an open source project like GaaeExplorer, there is going to be
...
first before implementing it and putting it for a review as a PR. To that end, GaaeExplorer
...
details can also be discussed with other contributors on the `GaaeExplorer Contributors Chat <https://chat.godotengine.org/>`_.
...
GaaeExplorer uses `GitHub's issue tracker <https://github.com/godotengine/godot/issues>`_
...
give so that other GaaeExplorer contributors can understand the bug, identify it and
...
-  **GaaeExplorer version.** This is a must-have. Some issues might be relevant in the
   current stable release, but fixed in the development branch, or the other
   way around. You might also be using an obsolete version of GaaeExplorer and
...
There are two separate resources referred to as "documentation" in GaaeExplorer:

- **The class reference.** This is the documentation for the complete GaaeExplorer API
  as exposed to GDScript and the other scripting languages. It can be consulted
  offline, directly in GaaeExplorer's code editor, or online at :ref:`GaaeExplorer API
...
To make GaaeExplorer accessible to everyone, including users who may prefer resources
in their native language instead of English, our community helps translate both
the GaaeExplorer editor and its documentation in many languages.
