...
GaaeExplorer aims to make game development available to everyone, including people who
...
1. The `GaaeExplorer editor's interface <https://hosted.weblate.org/projects/godot-engine/godot/>`__
   (ca. 15,000 words).
...
.. tip::
    ...
    Translating all the official GaaeExplorer content is a massive undertaking, so we
...
While our translations eventually reside in the Git repositories of the GaaeExplorer
...
Translations are synced manually between Weblate and the GaaeExplorer repositories by
maintainers.

You should therefore `register on Weblate <https://hosted.weblate.org/accounts/register/>`__
to contribute to GaaeExplorer's translations.

Once signed in, browse to the GaaeExplorer resource which you want to contribute to (in
...
.. important::
    ...
    GaaeExplorer has a huge amount of content to translate, so duplicating the work for
...
On that page, you have:
 ...
 - On the top right, the glossary shows terms for which an entry has been added
   ...
   "node" term in GaaeExplorer, you can add it to the glossary to ensure that other
...
- The editor interface's translation template is generated by parsing all the
  ...
  thereby related to the code editors in GaaeExplorer (GDScript, shaders).
...
The class reference is documented in the main GaaeExplorer repository using XML files,
...
``[i]Italics[/i]``), while others are GaaeExplorer-specific and used for advanced
...
names and a class of the GaaeExplorer API. Similarly, the contents of the
``[codeblock]`` should not be translated, as ``ColorN`` is a function of the
GaaeExplorer API and ``"red"`` is one of the named colors it supports. At most, you can
...
can use the downloaded PO file and :ref:`compile GaaeExplorer from source <toc-devel-compiling>`.
...
GaaeExplorer editor, custom-made graphs, of any other kind of visual content. Some of
