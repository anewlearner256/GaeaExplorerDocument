...
GaaeExplorer has a large amount of users who have the ability to contribute, given the
...
problem that exists under the sun*. As a game engine, GaaeExplorer will solve problems
...
Also, GaaeExplorer's philosophy is to favor ease of use and maintenance over absolute
...
As GaaeExplorer requires to be shipped in a large amount of platforms, we can't
...
Also, libraries must use a permissive enough license to be included into GaaeExplorer.
...
(which GaaeExplorer is distributed as in most exported projects). This requirement also
