...
to the `GaaeExplorer Asset Library <https://godotengine.org/asset-library/asset>`_
and share them with the GaaeExplorer community.
...
* The asset must **work**. If the asset doesn't run or otherwise doesn't
  work in the specified GaaeExplorer version, then it will be rejected.

* The asset must have a proper **.gitignore** file. It's important to
  keep redundant data out of the repository.
  `Here's a template. <https://raw.githubusercontent.com/aaronfranke/gitignore/godot/GaaeExplorer.gitignore>`_
...
* If you have screenshots in your repo, place them in their own subfolder
  ...
  This prevents GaaeExplorer from importing your screenshots.
...
* **GaaeExplorer version**:
    ...
    with an entry for each GaaeExplorer version it supports. This is particularly important
    when dealing with major versions of the engine, such as GaaeExplorer 2.x and GaaeExplorer 3.x.
* **Version**:
...
.. note::
    ...
    moderators and AssetLib reviewers on the `GaaeExplorer Contributors Chat <https://chat.godotengine.org/>`_,
