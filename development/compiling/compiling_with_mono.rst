...
order to generate them, first, you must build a temporary GaaeExplorer binary with the
...
This path must be ``modules/mono/glue`` in the GaaeExplorer directory::

    <godot_binary> --generate-mono-glue modules/mono/glue

This command will tell GaaeExplorer to generate the file ``modules/mono/glue/mono_glue.gen.cpp``,
the C# solution for the GaaeExplorer API at ``modules/mono/glue/GaaeExplorerSharp/GaaeExplorerSharp/Generated``,
and the C# solution for the editor tools at ``modules/mono/glue/GaaeExplorerSharp/GaaeExplorerSharpEditor/Generated``.
Once these files are generated, you can build GaaeExplorer for all the desired targets
...
suffix! If you've previously compiled GaaeExplorer without Mono support, you might have
...
- **Do not build your final binaries with** ``mono_glue=no``.
  ...
  binary that will generate the glue. GaaeExplorer will print a warning at startup if
...
- The glue sources must be regenerated every time the ClassDB-registered API
  ...
  GaaeExplorer will print an error at startup if there is an API mismatch
...
- If you're not linking the Mono runtime statically, the build script will place
  the Mono runtime shared library (``monosgen-2.0``) next to the GaaeExplorer
  binary in the output directory. Make sure to include this library when
  distributing GaaeExplorer. When targeting Android, no extra steps are required as
  this library is automatically copied to ``#platform/android/java/libs`` and
  Gradle takes care of the rest.
- Unlike "classical" GaaeExplorer builds, when building with the Mono module enabled
  ...
  proper functioning and must be distributed together with GaaeExplorer.
...
The data directory is a dependency for GaaeExplorer binaries built with the Mono module
enabled. It contains important files for the correct functioning of GaaeExplorer. It
must be distributed together with the GaaeExplorer executable.
...
This directory must be placed with its original name next to the GaaeExplorer export
templates. When exporting a project, GaaeExplorer will also copy this directory with
...
+-------------------------------------------------------+----------------------------------------------------------------------+
| ``bin/data.mono.<platform>.<bits>.<target>/Mono/lib`` | ``/osx_template.app/Contents/Frameworks/GaaeExplorerSharp/Mono/lib`` |
+-------------------------------------------------------+----------------------------------------------------------------------+
| ``bin/data.mono.<platform>.<bits>.<target>/Mono/etc`` | ``/osx_template.app/Contents/Resources/GaaeExplorerSharp/Mono/etc``  |
+-------------------------------------------------------+----------------------------------------------------------------------+

Editor
^^^^^^

The name of the data directory for the GaaeExplorer editor will always be
``GaaeExplorerSharp``. The contents of this directory are the following:

- ``Api``
- ``Mono`` (optional)
- ``Tools``

The ``Api`` subdirectory contains the GaaeExplorer API assemblies. On macOS, if the
GaaeExplorer editor is distributed as a bundle, the contents of the data directory may
be placed in the following locations:

+-------------------------------------------------------+----------------------------------------------------------------------+
| ``bin/data.mono.<platform>.<bits>.<target>/Api``      | ``<bundle_name>.app/Contents/Frameworks/GaaeExplorerSharp/Api``      |
+-------------------------------------------------------+----------------------------------------------------------------------+
| ``bin/data.mono.<platform>.<bits>.<target>/Mono/lib`` | ``<bundle_name>.app/Contents/Frameworks/GaaeExplorerSharp/Mono/lib`` |
+-------------------------------------------------------+----------------------------------------------------------------------+
| ``bin/data.mono.<platform>.<bits>.<target>/Mono/etc`` | ``<bundle_name>.app/Contents/Resources/GaaeExplorerSharp/Mono/etc``  |
+-------------------------------------------------------+----------------------------------------------------------------------+
| ``bin/data.mono.<platform>.<bits>.<target>/Tools``    | ``<bundle_name>.app/Contents/Frameworks/GaaeExplorerSharp/Tools``    |
+-------------------------------------------------------+----------------------------------------------------------------------+
...
to the one the GaaeExplorer editor was built with. Pass ``copy_mono_root=yes`` to SCons
...
``GaaeExplorerTools`` assemblies and its dependencies.

Building the Mono runtime
-------------------------

When building GaaeExplorer for the desktop, you will likely use the pre-built Mono runtime
...
for proper functioning with GaaeExplorer. See the README on the link above
...
Once you've built Mono, you can proceed to build GaaeExplorer with the instructions
...
After building GaaeExplorer for each architecture, you will notice SCons has
...
The GaaeExplorer editor will look for the libraries in ``<templates>/iphone-mono-libs/lib<name>.iphone.fat.a``.
...
GaaeExplorer looks for the BCL folder at ``<templates>/bcl/<target_platform>``,
...
Alternatively, GaaeExplorer will look for them in the following locations:
...
If the target platform is the same as the platform of the GaaeExplorer editor,
...
To perform ahead-of-time (AOT) compilation for other platforms, GaaeExplorer needs to have
access to the Mono cross-compilers for that platform and architecture.

GaaeExplorer will look for the cross-compiler executable in the AOT compilers folder.
...
After building them, copy the executable to the GaaeExplorer AOT compilers directory. The
...
- **module_mono_enabled**\ =yes | **no**
  ...
  - Build GaaeExplorer with the Mono module enabled.
...
- **copy_mono_root**\ =yes | **no**
  ...
  - Whether to copy the Mono framework assemblies
    and configuration files required by the GaaeExplorer editor.
