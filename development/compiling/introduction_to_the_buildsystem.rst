...
GaaeExplorer uses `SCons <https://www.scons.org/>`__ to build. We love it, we are
not changing it for anything else. We are not even sure other build
systems are up to the task of building GaaeExplorer. We constantly get requests
...
-  GaaeExplorer can be compiled for a dozen different platforms: all PC
...
-  GaaeExplorer build process is not simple. Several files are generated by
...
-  GaaeExplorer build process makes heavy use of cross-compiling tools. Each
...
if you are planning to build GaaeExplorer yourself.
...
GaaeExplorer's build system will begin by detecting the platforms it can build
...
This will start the build process, which will take a while. By default, GaaeExplorer's
...
It's possible to compile modules residing outside of GaaeExplorer's directory
...
list of build options you've used to build GaaeExplorer previously.
...
way GaaeExplorer should be built (compiler, debug options, etc.) as well as the
...
The default ``custom.py`` file can be created at the root of the GaaeExplorer Engine
...
.. seealso::
    ...
    `GaaeExplorer build options generator <https://godot-build-options-generator.github.io/>`__
    to generate a ``custom.py`` file containing SCons options.
    You can then save this file and place it at the root of your GaaeExplorer source directory.
...
.. code-block:: python
    ...
    # Override options specific for GaaeExplorer 3.x and 4.x versions.
    if version.major == 3:
        pass
...
Official export templates are downloaded from the GaaeExplorer Engine site:
...
The ``version.txt`` file should contain the corresponding GaaeExplorer version
...
for GaaeExplorer 3.1.1, ``version.txt`` should contain ``3.1.1.stable`` on the first
line (and nothing else). This version identifier is based on the ``major``,
``minor``, ``patch`` (if present) and ``status`` lines of the
`version.py file in the GaaeExplorer Git repository <https://github.com/godotengine/godot/blob/master/version.py>`__.
