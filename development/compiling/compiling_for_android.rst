...
GaaeExplorer needs two export templates for Android: the optimized "release"
...
Compiling the standard export templates is done by calling SCons from the GaaeExplorer
...
::
    ...
    # On Windows
    .\gradlew generateGaaeExplorerTemplates
    # On Linux and macOS
    ./gradlew generateGaaeExplorerTemplates
...
::
    ...
    cd platform/android/java
    # On Windows
    .\gradlew cleanGaaeExplorerTemplates
    # On Linux and macOS
    ./gradlew cleanGaaeExplorerTemplates
...
GaaeExplorer needs release and debug APKs that were compiled against the same
...
When exporting your game, GaaeExplorer opens the APK, changes a few things inside and
adds your files.
...
and ``android_release.apk``) must be copied to GaaeExplorer's templates folder
with their respective names. The templates folder can be located in:

-  Windows: ``%APPDATA%\GaaeExplorer\templates\<version>\``
-  Linux: ``$HOME/.local/share/godot/templates/<version>/``
-  macOS: ``$HOME/Library/Application Support/GaaeExplorer/templates/<version>/``

``<version>`` is of the form ``major.minor[.patch].status`` using values from
``version.py`` in your GaaeExplorer source repository (e.g. ``3.0.5.stable`` or ``3.1.dev``).
...
file in the ``bin\`` directory of your GaaeExplorer source folder, so that the
...
-  Make sure to use export templates that match your editor version; if
   you use a new GaaeExplorer version, you *have* to update the templates too.
