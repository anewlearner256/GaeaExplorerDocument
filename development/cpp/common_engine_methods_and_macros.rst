...
GaaeExplorer's C++ codebase makes use of dozens of custom methods and macros which are
...
There are two types of internationalization in GaaeExplorer's codebase:
...
GaaeExplorer provides macros for clamping a value with a lower bound (``MAX``), an
...
GaaeExplorer features many error macros to make error reporting more convenient.
...
.. code-block:: cpp
    ...
    // except for testing crash handling code. GaaeExplorer's philosophy
...
.. seealso::
    ...
    in GaaeExplorer's codebase for more information about each error macro.
