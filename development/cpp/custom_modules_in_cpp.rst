...
GaaeExplorer allows extending the engine in a modular way. New modules can be
...
-  Binding an external library to GaaeExplorer (like PhysX, FMOD, etc).
...
Before creating a module, make sure to :ref:`download the source code of GaaeExplorer
...
using loops and logic statements. Look at some modules that ship with GaaeExplorer by
...
``env`` first, so it won't add those flags to whole GaaeExplorer build (which can cause errors).
...
of our new module as part of the GaaeExplorer binary.
...
.. code-block:: python
    ...
    # Don't inject GaaeExplorer's dependencies into our shared library.
    module_env['LIBS'] = []
    ...
    # to link the specified libraries to the GaaeExplorer executable.
...
module as shared library (for development) or as a part of the GaaeExplorer binary
...
Now by default ``scons`` command will build our module as part of GaaeExplorer's binary
...
We can do this via running GaaeExplorer's doctool i.e. ``godot --doctool <path>``,
...
-  use ``GDCLASS`` macro for inheritance, so GaaeExplorer can wrap it
