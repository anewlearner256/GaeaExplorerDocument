...
To optimize GaaeExplorer's performance, you need to know what to optimize first.
...
To get useful profiling information, it is **absolutely required** to use a GaaeExplorer
...
If you're looking into optimizing GaaeExplorer's startup/shutdown performance,
you can tell the profiler to use the ``--quit`` command line option on the GaaeExplorer binary.
This will exit GaaeExplorer just after it finished starting.
...
- Start the GaaeExplorer editor or your project first.
...
- Open VerySleepy and select the GaaeExplorer executable in the list of processes on the left:
...
- Once the results window appears, filter the view to remove external modules (such as the graphics driver).
  You can filter by module by finding a line whose **Module** matches the GaaeExplorer
  executable name, right-clicking that line then choosing
  **Filter Module to <GaaeExplorer executable name>** in the dropdown that appears.
...
- In the next window, specify the path to the GaaeExplorer binary that includes debug symbols.
...
- Make sure **Elevate Privileges** is checked if you have administrative privileges.
  While not essential for profiling GaaeExplorer, this will ensure all events can be captured.
...
- In the Time Profiler window, click on the **Target** menu, select **Choose target...**
  and specify the path to the GaaeExplorer binary, command line arguments and environment variables
  in the next window.
