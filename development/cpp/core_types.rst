...
GaaeExplorer has a rich set of classes and templates that compose its core,
...
GaaeExplorer uses the standard C99 datatypes, such as ``uint8_t``,
...
GaaeExplorer ensures that all objects that can be allocated dynamically are
...
GaaeExplorer has the option to use a dynamic memory pool. This memory needs to
...
GaaeExplorer has many tools for tracking memory usage in a game, especially
during debug. Because of this, the regular C and C++ library calls
should not be used. Instead, a few other ones are provided.

For C-style allocation, GaaeExplorer provides a few macros:
...
GaaeExplorer provides also a set of common containers:
...
GaaeExplorer also provides a String class. This class has a huge amount of
